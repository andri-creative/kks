...
export default function Settings() {
    ...
    // Sync form data if settings change externally
    useEffect(() => {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50/30 overflow-y-auto pb-10">
            {/* Toast Notification */}
            {toast && (
                <div className={`fixed bottom-5 right-5 z-50 flex items-center gap-2.5 px-4 py-3 rounded-xl shadow-lg border transition-all duration-300 transform translate-y-0 animate-in fade-in slide-in-from-bottom-2 ${
                    toast.type === 'success' 
                ...
                <form onSubmit={handleSave} className="bg-white rounded-xl border border-gray-100 shadow-md p-6 space-y-6 w-full text-left">
                    ...
                    <div className="flex border-b border-gray-100 pb-3 gap-6 overflow-x-auto custom-scrollbar">
                        ...
                        <button
                            ...
                            className={`pb-2 text-xs font-black uppercase tracking-wider outline-none transition-all duration-300 relative cursor-pointer flex items-center gap-2 ${
                                activeTab === 'profile'
                                    ? 'text-text-green'
